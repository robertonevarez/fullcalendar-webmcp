...
export async function seedDatabase(force = false) {
  return runInTransaction(async () => {
    ...
    await insertBusiness(
      ...
      'Reliable residential cleaning across the El Paso area. Eco-friendly products, flexible scheduling, and a team that treats your home with care.',
      'America/Denver',
      'CUSTOMER_LOCATION',
    ...
    await insertService(
      ...
      'biz_marias_cleaning',
      'Weekly Cleaning',
      'Scheduled weekly maintenance visits to keep kitchens, baths, and living areas consistently tidy between deep cleans.',
      90,
      10000,
      'USD',
      JSON.stringify(['weekly cleaning', 'recurring cleaning', 'maintenance cleaning', 'scheduled cleaning']),
      'CUSTOMER',
      true,
      JSON.stringify([{ resource_type: 'cleaner', quantity: 1, capability: 'standard_cleaning' }]),
      JSON.stringify({
        fields: ['property_type', 'bedrooms', 'bathrooms', 'preferred_day', 'entry_instructions'],
        eligibility_rules: {
          property_types_supported: ['house', 'apartment', 'condo', 'townhouse'],
          max_bedrooms: 6,
          max_bathrooms: 4,
    ...
    await insertService(
      'svc_post_construction_cleaning',
      'biz_marias_cleaning',
      'Post-Construction Cleaning',
      'Dust, debris, and finish cleanup after renovations — windows, floors, fixtures, and surfaces ready for living again.',
      300,
      35000,
      'USD',
      JSON.stringify([
      ...
      JSON.stringify([{ resource_type: 'cleaner', quantity: 1, capability: 'deep_cleaning' }]),
      JSON.stringify({
        fields: ['property_type', 'renovation_areas', 'construction_complete_confirmed', 'entry_instructions'],
        eligibility_rules: {
          property_types_supported: ['house', 'apartment', 'condo', 'townhouse'],
          max_bedrooms: 8,
          max_bathrooms: 6,
          ...
          requires_water_and_power: true,
          general_requirements: [
            'Construction or renovation work complete and major debris removed',
            'Access to property with active water and electricity',
          ],
        },
      }),
